...
func writeBech32Checksum(hrp string, data []byte, bldr *strings.Builder,
	...
	bech32Const := int(Bech32VersionToConsts[version])
...
// bech32m) is also retruend so allow the caller to perform proper address
...
func bech32VerifyChecksum(hrp string, data []byte) (Bech32Version, bool) {
	...
	// constants.
	bech32Version, ok := Bech32ConstsToVersion[Bech32Const(polymod)]
	if ok {
...
func decodeNoLimit(bech string) (string, []byte, Bech32Version, error) {
...
func DecodeGeneric(bech string) (string, []byte, Bech32Version, error) {
	// The maximum allowed length for a bech32 string is 90.
	if len(bech) > 90 {
		return "", nil, VersionUnknown, ErrInvalidLength(len(bech))
	}
...
func encodeGeneric(hrp string, data []byte,
	version Bech32Version) (string, error) {
